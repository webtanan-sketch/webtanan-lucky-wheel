...
class WTLW_Rewards {
	...
	public function apply( array $section, $user_id ) {
		...
		if ( 'extra_attempts' === $type || $extra_attempts > 0 ) {
			...
			if ( $this->woocommerce->is_available() ) {
				$coupon_code = $this->woocommerce->create_coupon( $user_id, $value, isset( $section['discount_type'] ) ? $section['discount_type'] : 'fixed_cart', $expiry_days );
				if ( ! $coupon_code ) {
					$status = 'failed';
				} else {
					$sms = $this->sms->send_user_coupon( $user_id, $coupon_code, $expiry_days, isset( $section['name'] ) ? $section['name'] : '' );
					$sms_sent = ! empty( $sms['sent'] );
				}
			} else {
		...
		return array( 'coupon_code' => $coupon_code, 'status' => $status, 'value' => $value, 'sms_sent' => $sms_sent );
	}
	...
	public function apply_guest( array $section, $participant_id ) {
		...
		if ( 'extra_attempts' === $type || $extra_attempts > 0 ) {
			...
			if ( $this->woocommerce->is_available() ) {
				$coupon_code = $this->woocommerce->create_guest_coupon( $participant_id, $value, isset( $section['discount_type'] ) ? $section['discount_type'] : 'fixed_cart', $expiry_days );
				if ( ! $coupon_code ) {
					$status = 'failed';
				} else {
					$sms = $this->sms->send_participant_coupon( $participant_id, $coupon_code, $expiry_days, isset( $section['name'] ) ? $section['name'] : '' );
					$sms_sent = ! empty( $sms['sent'] );
				}
			} else {
