...
class WTLW_Appearance {
	...
	public function menu() {
		add_submenu_page(
			'wtlw-dashboard',
			__( 'ظاهر و رنگ‌بندی', 'webtanan-lucky-wheel' ),
			__( 'ظاهر و رنگ‌بندی', 'webtanan-lucky-wheel' ),
			'manage_options',
			'wtlw-appearance',
			array( $this, 'render' )
	...
	public function render() {
		...
		$fields = array(
			'primary'    => 'رنگ اصلی',
			'secondary'  => 'رنگ دوم',
			'accent'     => 'رنگ طلایی / تأکیدی',
			'background' => 'پس‌زمینه اصلی',
			'surface'    => 'سطح کارت‌ها و فرم',
			'text'       => 'رنگ متن اصلی',
			'muted'      => 'رنگ متن فرعی',
		);
		?>
		<div class="wrap wtlw-admin-wrap" dir="rtl">
			...
			<?php if ( isset( $_GET['updated'] ) ) : ?>
				<div class="notice notice-success is-dismissible"><p><?php echo esc_html__( 'رنگ‌بندی ذخیره شد.', 'webtanan-lucky-wheel' ); ?></p></div>
			<?php endif; ?>
			...
			<div style="display:grid;grid-template-columns:minmax(0,620px) minmax(280px,1fr);gap:22px;max-width:1050px;align-items:start">
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="wtlw-admin-panel" style="padding:24px;background:#fff;border:1px solid #ddd;border-radius:14px">
					...
					<?php foreach ( $fields as $key => $label ) : ?>
						<label style="display:grid;gap:7px;font-weight:600">
							<?php echo esc_html( $label ); ?>
							<span style="display:flex;gap:10px;align-items:center">
								<input type="color" name="<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $colors[ $key ] ); ?>" style="width:52px;height:38px;padding:2px" />
								<code dir="ltr"><?php echo esc_html( $colors[ $key ] ); ?></code>
							</span>
						</label>
					<?php endforeach; ?>
					</div>
					...
					<p style="margin-top:22px">
						<button class="button button-primary button-large" type="submit"><?php echo esc_html__( 'ذخیره رنگ‌بندی', 'webtanan-lucky-wheel' ); ?></button>
						<button class="button button-large" type="submit" name="reset_altin" value="1"><?php echo esc_html__( 'بازگشت به پالت پیشنهادی آلتین واچ', 'webtanan-lucky-wheel' ); ?></button>
					</p>
				</form>
				...
				<div class="wtlw-admin-panel" style="padding:24px;border-radius:14px;background:<?php echo esc_attr( $colors['background'] ); ?>;color:<?php echo esc_attr( $colors['text'] ); ?>;border:2px solid <?php echo esc_attr( $colors['accent'] ); ?>">
					<small style="color:<?php echo esc_attr( $colors['accent'] ); ?>"><?php echo esc_html__( 'پیش‌نمایش سریع', 'webtanan-lucky-wheel' ); ?></small>
					<h2 style="color:inherit"><?php echo esc_html__( 'گردونه شانس آلتین', 'webtanan-lucky-wheel' ); ?></h2>
					<p style="color:<?php echo esc_attr( $colors['muted'] ); ?>"><?php echo esc_html__( 'ترکیب فعلی رنگ‌های قالب در این کارت نمایش داده می‌شود.', 'webtanan-lucky-wheel' ); ?></p>
					<div style="padding:15px;border-radius:12px;background:<?php echo esc_attr( $colors['surface'] ); ?>;border:1px solid <?php echo esc_attr( $colors['secondary'] ); ?>"><?php echo esc_html__( 'نام و شماره موبایل', 'webtanan-lucky-wheel' ); ?></div>
					<div style="margin-top:14px;padding:11px;text-align:center;border-radius:10px;background:<?php echo esc_attr( $colors['accent'] ); ?>;color:<?php echo esc_attr( $colors['background'] ); ?>;font-weight:800"><?php echo esc_html__( 'شرکت در قرعه‌کشی', 'webtanan-lucky-wheel' ); ?></div>
				</div>
			</div>
		</div>
		<?php
